...
model = LogisticRegression(max_iter=500,
                           solver="newton-cg")  # create a logistic regression object, you can specify parameters such as regularization term, learning rate etc., default using L2 regularization and liblinear solver.
model.fit(X_train,
          y_train)  # use the features and labels of the training set to fit the model, i.e. solve for model parameters.
...
acc = accuracy_score(y_test,
                     y_pred)  # use accuracy_score function to calculate accuracy score between prediction results and true labels, i.e. proportion of correct predictions.
print("Accuracy score:", acc)  # print accuracy score.

# Plot comparison between prediction results and true labels
plt.scatter(range(len(y_test)), y_test,
            label="True labels")  # use scatter function to plot scatter plot, x-axis is sample index number, y-axis is true label (0 or 1), and set legend.
plt.scatter(range(len(y_pred)), y_pred,
            label="Prediction results")  # same as above, y-axis is prediction result (0 or 1).
